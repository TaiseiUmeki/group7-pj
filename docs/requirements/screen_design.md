...
| 2 | 新規登録 | `/signup` | 不要 | 未ログインユーザー | メールアドレスとパスワードでアカウントを作成する |
| 2.5 | 初回プロフィール登録 | `/profile-setup` | 必要 | ユーザー | 表示名、自己紹介、タグ、トレーニング頻度を登録する |
...
| 8 | プロフィール編集 | `/me/edit` | 必要 | ユーザー | 表示名、自己紹介、タグ、トレーニング頻度を編集する |
...
graph TD
    ...
    Signup[新規登録] --> ProfileSetup[初回プロフィール登録]
    ProfileSetup --> Timeline
...
| 2 | 自己紹介 | text | 任意 | `bio` |
| 3 | タグ | list | 任意 | `profile_tags.tag_id` の表示値 |
...
| 3 | タグ | int list | 任意 | `profile_tags.tag_id` |
...
| 2 | タグ取得 | 初期表示 | タグIDと表示名を取得 | `GET /api/masters/profile-tags` |
...
対象候補からランダムに推薦された最大5人のユーザーを表示する。表示中の推薦ユーザーは全員フォローできる。
...
| # | 項目名 | 型 | 必須 | 説明 |
|---|---|---|---|---|
| 1 | 通知種別 | int label | 必須 | 応援、サボり検知。いいね通知は表示しない |
...
| UserCard | 推薦ユーザー、フォロー導線 | ユーザー名、タグ、フォローボタンを表示 |
