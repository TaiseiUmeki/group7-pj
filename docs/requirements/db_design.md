...
- ステータスや種別は `int` のIDとして管理し、IDに対応する表示値はバックエンドで変換する。
  - 例: `profile_tags.tag_id = 2` を「大会勢」として扱う。
...
- プロフィールタグは `profile_tags.profile_id` と `profile_tags.tag_id` の組み合わせを一意にする。
...
ユーザーの表示情報、プロフィールタグ、トレーニング頻度を管理する。
プロフィールタグは `profile_tags` で複数保持し、推薦ユーザー抽出には利用しない。
...
プロフィールに紐づく固定タグIDを管理する。
タグ名はDBに持たず、バックエンドの定数でIDから表示値へ変換する。
タグはプロフィール表示用であり、推薦・マッチング条件には使用しない。
...
| カラム名   | 型       | NULL | PK  | FK          | デフォルト        | 説明             |
| ---------- | -------- | ---- | --- | ----------- | ----------------- | ---------------- |
| id         | bigint   | NO   | YES |             | auto increment    | プロフィールタグID |
| profile_id | bigint   | NO   |     | profiles.id |                   | プロフィールID   |
| tag_id     | int      | NO   |     |             |                   | 固定タグID       |
| created_at | datetime | NO   |     |             | current timestamp | 作成日時         |
...
| インデックス名          | カラム             | 種別   | 目的                     |
| ----------------------- | ------------------ | ------ | ------------------------ |
| uk_profile_tags_pair    | profile_id, tag_id | unique | 同一タグの重複設定防止   |
| idx_profile_tags_profile | profile_id         | normal | プロフィール別タグ取得   |
| idx_profile_tags_tag_id  | tag_id             | normal | タグ別ユーザー検索の拡張 |
...
### 3.8 recommendation_slots

日替わり推薦ユーザー枠を管理する。
表示中の推薦ユーザーは全員フォロー可能であり、推薦枠からのフォロー人数に1日あたりの上限は設けない。
推薦対象はタグやプロフィール条件に依存せず、対象候補からランダムに抽出する。
...
| notification_type  | int      | NO   |     |                     |                   | 通知種別ID。例: 1=応援、2=サボり検知。いいね通知は作成しない |
...
| profiles          | profile_tags         | 1:N    | profile_tags.profile_id                  | プロフィールは複数タグを持つ             |
...
| 002 | `profiles` 作成             | `users`                                       | プロフィール、トレーニング頻度         |
| 003 | `profile_tags` 作成         | `profiles`                                    | プロフィールタグ                       |
| 004 | `training_sessions` 作成    | `users`                                       | クイックスタート                       |
| 005 | `training_posts` 作成       | `users`, `training_sessions`                  | 事後報告、投稿詳細、ログ、カレンダー   |
| 006 | `follows` 作成              | `users`                                       | フォロー、TL                           |
| 007 | `post_likes` 作成           | `users`, `training_posts`                     | いいね                                 |
| 008 | `recommendation_slots` 作成 | `users`                                       | 日替わり推薦ユーザー                   |
| 009 | `support_messages` 作成     | `users`                                       | がんばれボタン押下記録                 |
